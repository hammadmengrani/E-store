...
const Checkout = () => {
  ...
  const { cartItems, clearCart } = useCart(); // Get cart items from context
  ...
  const handleOrderComplete = async (event: React.FormEvent) => {
    ...
    try {
      // 🔹 Prepare Items for the Order (Ensure correct structure)
      const items = cartItems.map((item) => ({
        productId: item.id,  // ✅ Ensure `productId` is sent
        title: item.title,
      ...
      const orderData = {
        ...
        billingAddress: isDifferentBillingAddress
          ...
          : null,
        items,  // ✅ Correctly formatted items
        totalAmount,  // ✅ Ensure total amount is included
      };
  
      // 🔹 Call GraphQL Mutation (Add Order)
