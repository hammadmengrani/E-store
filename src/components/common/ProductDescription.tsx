...
const ProductDescription = (props: productDetail) => {
    ...
    const handleAddToCart = () => {
        if (!selectedAttribute) return;  // Prevent adding if no attribute selected
    ...
    return (
        <div className='flex flex-col gap-4 md:px-0 px-5'>
            ...
            <Discount rate={props.price} className='text-xl' discount={props.discount} />

            {/* Description */}
            <p className="text-gray-600 md:w-[850px]">{truncateDesc(props.description, 150)}</p>
                    ...
                    {props.variations.map((attr, index) => (
                        <button 
                            ...
                            className={`px-3 py-1 rounded-full text-sm transition 
                                ${selectedAttribute === attr ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
                        >
                            {attr}
                        </button>
                    ))}
            ...
            <button
                ...
                className={`mt-4 flex w-40 text-center items-center justify-center text-white px-6 py-2 rounded-full transition duration-300 
                    ${selectedAttribute ? 'bg-[#059DDE] hover:bg-blue-700' : 'bg-gray-400 cursor-not-allowed'}`}
            >
                Add to Cart
            </button>
            ...
            <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-2 mt-1">
                    {props.category.map((cat, index) => (
                        <span key={index} className="px-3 py-1 bg-green-200 text-green-700 rounded-full text-sm">
                            {cat}
                        </span>
                    ))}
                </div>
